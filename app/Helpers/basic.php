...
function create_list($list, $id_parent = 0){
	if($id_parent==0)
	...
	foreach($list as $key => $value){
		if($value['id_parent'] == $id_parent){
			$subhtml = create_list($list, $value['id']);
			$html .= '<li>';
				$html .= '<a href="'.$value['slug'].'" title="'.$value['title'].'">'.$value['title'];
				if($subhtml)
...
function showTrees($items, $checkeds, $name = 'cate_id[]', $type = 'checkbox', $titledefault = 'Danh mục chính', $disabled = false){
	...
	<div class="item" style="padding-left:0px">
		<label><input type="'.$type.'" class="'.$type.'" name="'.$name.'" value="0" checked'.(($disabled)?' disabled':'').'>'.$titledefault.'</label>
	</div>
	';
...
function date_vi( $format, $time = 0 )
{
    if ( ! $time ) $time = time();
