	...
	<div class="row-fluid">
		<div class="span12 text-right text-top">
			<div class="actionfull">
				@ability('root', 'c_role')
				<a href="{{ route('backend.permission.create') }}" class="btn btn-primary"><i class="iconfa-plus"></i> Thêm mới</a>
				@endability
				@ability('root', 'd_role')
				<button class="btn btn-danger btn-deletes" onCLick="return deleteItems('nhóm', '{!! route('backend.permission.deletes', 'role') !!}')"><i class="iconfa-trash"></i> Xóa nhiều</button>
				@endability
			</div>
		</div>
	</div>
			...
			<div id="table-header" class="row-fluid">
				...
				<table class="table table-bordered">
					...
					<tbody id="tablecontent">
						...
						<tr>
							<td colspan="20">Not found permission ... </td>
						</tr>
						@else
							...
							<tr>
								...
								<td style="width:10%;" class="action">
									<a href="{!! route('backend.permission.view', $value->id) !!}" title="Xem chi tiết" data-toggle="tooltip" class="btn-read"><span class="iconfa-eye-open muted"></span></a>
									@ability('root', 'u_role')
									<a href="{{ route('backend.permission.edit', $value->id) }}" title="Chỉnh sửa" data-toggle="tooltip" class="btn-update"><span class="iconfa-edit muted"></span></a>
									@endability
									@ability('root', 'd_role')
									<a href="javascript: void(0)" title="Xóa" data-toggle="tooltip" class="btn-delete" onClick="return deleteItem('Khi bạn đồng ý xóa thì tất cả dữ liệu của quyền này sẽ <span class=&quot;text-error&quot;>bị xóa vĩnh viễn</span>.<br/><br/><p class=&quot;text-error text-center&quot;>Bạn có chắc vẫn muốn xóa quyền [<strong>{!! $value->description !!}</strong>]?</p>', '{!! route('backend.permission.delete', $value->id) !!}')"><span class="iconfa-trash muted"></span></a>
									@endability
								</td>
							</tr>
							@endforeach
